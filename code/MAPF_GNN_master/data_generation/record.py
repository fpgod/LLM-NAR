...
def record_env(path, config):
    cases = [f for f in os.listdir(path) if f.startswith('case_')]
    t = np.zeros(len(cases))

    for i in range(len(cases)):
        
        trayectory = np.load(
            os.path.join(path, rf"case_{i}/trajectory.npy"), allow_pickle=True
        )
    ...
    for timestep in range(len(cases)):
        if os.path.exists(os.path.join(path, rf"case_{timestep}/states.npy")):
            continue
        # print("case:", timestep)
        ...
        # mx = env.min_time
        trayectory = np.load(
            os.path.join(path, rf"case_{timestep}/trajectory.npy"), allow_pickle=True
        )
        trayectory = trayectory[:, 1:] #去除掉0时刻数据
        recordings = np.zeros(
        ...
        print("trayectory", trayectory.shape)
        ...
        print("trayectory", trayectory.shape)  #[num_agents, time-1]
        print("recordings", recordings.shape)
        print("adj_record", adj_record.shape)
        ...
        np.save(
            os.path.join(path, rf"case_{timestep}/trajectory_record.npy"), trayectory  
        )
        if timestep % 25 == 0:
