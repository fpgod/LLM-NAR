...
def parse_trayectories(schedule):
    ...
    # print("trayect", trayect.shape)  #[num_agents, time]
    # print("startings", np.array(startings)) #[num_agents, 2]

    return trayect, np.array(startings)


def parse_traject(path):
    case_folders = [f for f in os.listdir(path) if f.startswith('case_')]
    cases = len(case_folders)
    # print(cases)
    print("Parsing trayectories")
    for i in range(len(case_folders)):
        if os.path.exists(os.path.join(path, rf"case_{i}/trajectory.npy")):
            continue
        # print("case:", i)
        ...
        np.save(os.path.join(path, rf"case_{i}/trajectory.npy"), t)
        np.save(os.path.join(path, rf"case_{i}/startings.npy"), s)

        if i % 25 == 0:
            print(f"Trajectoty -- [{i}/{len(case_folders)}]")
    print(f"Trayectoty -- [{i}/{len(case_folders)}]")
